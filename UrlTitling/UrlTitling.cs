        ...
        foreach (string s in e.MessageArray)
        {
            if (s.StartsWith("http://") || s.StartsWith("https://"))
            {
                ...
                if (nickDisable.IsNickDisabled(e.Nick, e.Channel))
                    ...
                    ChannelThreadManager.EnqueueUrl(e.Channel, s);
            }
        }
    }
}
...
static class ChannelThreadManager
{
    ...
    static public void EnqueueUrl(string channel, string url)
    {
        ChannelThread thread = GetThread(channel);

        lock (thread._channelLock)
        {
            thread.UrlQueue.Enqueue(url);
    ...
    class ChannelThread
    {
        ...
        void Consumer(string url)
        {
            if (Blacklist.InBlacklist(url, Channel))
            {
